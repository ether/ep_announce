# Agent Guide — ep_announce

Plays a chime when other users join on Etherpad.
...
`ep_announce` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_announce`:
...
* Client: `postAceInit`, `userJoinOrUpdate`, `userLeave`
