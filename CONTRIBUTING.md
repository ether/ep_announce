# Contributing to ep_announce

Thanks for helping improve `ep_announce` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_announce` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_announce from your local clone
pnpm run plugins i --path ../ep_announce

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_announce/` are picked up after a server restart.
...
* Etherpad version, ep_announce version, browser + OS
* Steps to reproduce
